...
const DIVER_N1 = parse(Float64, get(ARGS, 3, "0.001"))
const DIVER_N2 = 0.01
...
function label_cells!(cell_matrix::AbstractMatrix{Int},cell_id, mutant_set)
    mut1=0
    mut2=0
    mut3=0
    for i in 1:size(cell_matrix, 1)
        for j in 1:size(cell_matrix, 2)
            if cell_matrix[i, j] != 0 
                ...
                cell_mutation_id = divition_ids(cell_id[cell_matrix[i, j]])   
                is_in_mutant1 =  mutant_set[1] in cell_mutation_id 
                is_in_mutant2 =  mutant_set[2] in cell_mutation_id 
                
                if is_in_mutant1 && is_in_mutant2
                    cell_matrix[i, j] = 4
                    mut3 += 1
                    
                    
                elseif is_in_mutant1 
                    cell_matrix[i, j] = 2
                    mut1 += 1
                
                elseif is_in_mutant2 
                    cell_matrix[i, j] = 3
                    mut2 += 1
    ...
    return cell_matrix,mut1,mut2,mut3
end
...
function plot_scatter(cell_matrix,cell_id,wx,possion_value,mut,mutant_set)
    ...
    for sampling_length in [10]#50,60]
...
function main(end_population=100000)
    global seed_value
    ...
    while mut + primary < end_population
        ...
        if event == :divide
            ...
            if (sum(mut) + primary)/end_population == DIVER_N1 && cell_b_r == 2 && check_if_mutated == 0
                print("mut1!")
                cell_b_r = 1
                check_if_mutated = 1
                push!(mutant_id,num_divi+2)
            end

            if (sum(mut) + primary)/end_population == DIVER_N2 && check_if_mutated == 1
                 print("mut2!"*string(sum(mut))*"->"*string(cell_b_r))
                if cell_b_r == 1
                    print("found!")
                end
                cell_b_r = 1
    ...
    if  0 <= mut/(mut+primary)
    	...
    	mb_fig=heatmap(mb_matrix,
        	color=cmap,
            title="F_mut="*string(mut/end_population),
            framestyle=:none,
            size=(800, 800),
            colorbar=true)        
    	...
    	wt_mut,m1,m2,m3 = label_cells!(cell_matrix,cell_id, mutant_id)
    	cmap = [:white, :blue, :red, :orange,:purple]
    
        hm_fig=heatmap(wt_mut,
        color = cmap,
        clims = (0, 4),          
        colorbar_ticks=[0, 1, 2, 3, 4],
        title="mut1="*string(m1/end_population)*" mut2="*string(m2/end_population)*" mut3="*string(m3/end_population),
        framestyle=:none,
        size=(800, 800),
        colorbar=false)                                  
        savefig(hm_fig, "/YOUR_PATH/" * string(PUSH_RATE)* "/" * string(POSSION_MUTATION) * "/heatmap_" * string(seed_value) * "_" * string(m1/100000) * "_" * string(m2/100000) * "_" * string(PUSH_RATE) * ".pdf")
    end
    return cell_matrix, primary, mut
end
